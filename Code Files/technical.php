...
if(isset($_REQUEST['sub']))
{
	...
	if($os == "" || $os == NULL)
	{
		header("location:technical.php?msg=Operating Systems must be filled out");
	}
	else if($lan == "" || $lan == NULL)
	{
		header("location:technical.php?msg=Computer Languages must be filled out");
	}
	if($techno == "" || $techno == NULL)
	{
		header("location:technical.php?msg=Technologies must be filled out");
	}
	else if($known == "" || $known == NULL)
	{
		header("location:technical.php?msg=Known Databases must be filled out");
	}
	else{
		$sql = "insert into technical set user_id = '".$user_id."', os = '".$os."', language = '".$lan."', technique = '".$techno."', known = '".$known."'";
		mysqli_query($con,$sql);
		header("location:others.php?id=$user_id");
	}
}
?>
...
<div class="col-md-12" style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif;">
            ...
            <div class="col-md-6">
            	<form method="post" name="form" onsubmit="return checkTechnical(form);">
                    <fieldset style="background:rgba(0.0,0.0,0.0,0.5); border:white 3px solid; padding:20px; margin-top:70px;">
                        <h2 style="color:white; font-weight:bold; margin-bottom:10px;" align="center">Technical Proficiency</h2>
                        <?php
						$id = $_REQUEST['id'];
						?>
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                            	<label for="exampleInputText1" style="color:white;">Operating Systems</label>
                                <input type="text" name="t1" class="form-control txt" required />
                            </div>
                        </div>
                        <div class="col-md-12">
                        	<div class="form-group">
                            	<label for="exampleInputText2" style="color:white;">Computer Languages</label>
                                <input type="text" name="t2" class="form-control txt" required />
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                            	<label for="exampleInputText3" style="color:white;">Technologies</label>
                                <input type="text" name="t3" class="form-control txt" required />
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                            	<label for="exampleInputText4" style="color:white;">Database</label>
